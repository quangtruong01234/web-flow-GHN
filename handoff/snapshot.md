# Handoff Snapshot — Plan & Frontend Reference (merged)

> **Merged 2026-07-05** from the two original handoff files:
> `ghn-next-admin-plan.md` (Prompt 1 context handoff, generated 2026-06-27) and
> `frontend-reference.md` (main TryBuy frontend pattern reference).
>
> **Historical snapshot.** Parts of Part 1 describe a pre-scaffold state ("no Next.js
> app, no package.json") that is long resolved — the app now lives at the repo root and
> is fully wired to the gateway. Current truth lives in `.ai/` (start at
> `.ai/project.md`). Done work is logged in `handoff/CHANGELOG.md`.

---

# Part 0 — Proposed improvement backlog (added 2026-07-05)

Findings from a repo audit on 2026-07-05, **beyond** the tasks already tracked in
`../.agent-local/frontend-handoff-ghn.md` (Open + "Recommended next improvements") and
`.ai/context/risks.md`. Ordered by priority. Move an item to `handoff/CHANGELOG.md` when
done.

1. ~~**App Router error/loading boundaries**~~ — **Done 2026-07-05** (see
   `handoff/CHANGELOG.md`): `(app)/error.tsx` + `(app)/loading.tsx` + `global-error.tsx`.
2. **CI pipeline** — no `.github/workflows`. Lint/typecheck/jest/build run only when
   someone remembers locally; add a workflow that runs them on every push/PR.
3. **URL-synced filters/pagination on `/shipments`** — no `useSearchParams` outside the
   login card; refresh or shared link loses filter/page state and Back doesn't restore.
   Sync table filters to query params for deep-linking.
4. **Data freshness for an ops console** — `queryClient.ts` sets
   `refetchOnWindowFocus: false` and nothing polls, so operators see stale statuses until
   they act. Consider refetch-on-focus or a gentle `refetchInterval` on list/detail.
5. **Modal accessibility** — hand-rolled `Modal.tsx` has Escape + scroll-lock but no focus
   trap, no initial focus, no focus restore on close; Tab escapes into the background.
   Per project rules, ask before adding shadcn Dialog vs. a small focus-trap patch.
6. **`next lint` deprecation** — lint warns it will be removed in Next 16; migrate to the
   ESLint CLI (`next-lint-to-eslint-cli` codemod) before it blocks an upgrade.
7. ~~**Security headers**~~ — **Done 2026-07-05** (see `handoff/CHANGELOG.md`):
   `next.config.mjs` now sends `X-Content-Type-Options`, `X-Frame-Options`,
   `Referrer-Policy`, `Permissions-Policy`. CSP deferred (needs nonce plumbing).
8. ~~**Pin Node version**~~ — **Done 2026-07-05:** `engines` (`>=20 <23`) in
   `package.json` + `.nvmrc` (22).

---

# Part 1 — GHN Next.js Admin Plan (original `ghn-next-admin-plan.md`)
...
# Part 2 — Frontend Reference Patterns (original `frontend-reference.md`)
